<?php
  ...
  if ($login && $password) {
    $sql = "SELECT * FROM users WHERE name = '" . $mysqli->real_escape_string($login) . "'";
    $result = $mysqli->query($sql);

    if ($result->num_rows <= 0) {
      $sql = "INSERT INTO users (name, password) VALUES ('" . $mysqli->real_escape_string($login) . "', '" . sha1($mysqli->real_escape_string($password)) . "')";
      $result = $mysqli->query($sql);
      ...
      <div>
        <div class="flex bg-red-400 bg-opacity-30 p-4 text-sm text-black dark:text-white" role="alert">
          ...
          <div>Takie konto już istnieje!</div>
        </div>
      </div>
      <?php
    }
...
<head>
    ...
    <title>Rejestracja - Oceny restauracji</title>
...
<body class="h-full w-full bg-gray-200 dark:bg-gray-900 dark:text-white">
  <div class="flex items-start justify-center md:mt-10">
    <div class="mx-auto max-w-2xl w-full dark:bg-gray-800 bg-white rounded-md shadow-lg p-8">
      ...
      <div>
        <form class="flex flex-col space-y-2" action="register.php" method="post">
          <input type="text" name="name" required autocomplete="off" placeholder="Nazwa użytkownika" class="w-full h-10 px-2 rounded-md bg-gray-50 dark:bg-gray-900 dark:text-white border border-gray-300 dark:border-gray-900">
          ...
          <button type="submit" class="w-32 h-10 bg-black text-white uppercase font-bold px-4 rounded-md">Zarejestruj</button>
        </form>
      </div>
    </div>
  </div>
